...
interface FooterProps {
  onOpenPopup: () => void;
  content: IcpContent['footer'];
}

export const Footer: React.FC<FooterProps> = ({ onOpenPopup, content }) => {
  return (
    <Section className="text-center pt-32 pb-32">
      <h2 className="text-4xl md:text-5xl font-sans font-bold text-charcoal mb-12 max-w-3xl mx-auto leading-tight">
        {content.headline}
        <br />{' '}
        <span className="text-high-vis-orange underline decoration-4 underline-offset-4">
          {content.highlight}
        </span>
      </h2>

      <div className="flex flex-col items-center gap-6">
        <Button variant="ghost" onClick={onOpenPopup}>{content.ctaLabel}</Button>
