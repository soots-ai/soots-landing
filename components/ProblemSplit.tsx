  ...
  return (
    <Section className="bg-white" noGrid>
      ...
      <div className="text-center max-w-3xl mx-auto mb-16 pt-12">
        <h2 className="font-sans font-bold text-3xl md:text-4xl lg:text-5xl text-charcoal mb-4">{content.title}</h2>
      ...
      <div className="relative w-full max-w-5xl mx-auto h-[400px] md:h-[500px] mb-20 px-2 md:px-4">
        ...
        <div className="w-full h-full relative border-l-2 border-b-2 border-charcoal">
            ...
            <div className="absolute bottom-[-30px] md:bottom-[-40px] left-0 w-full flex justify-between font-mono text-[9px] md:text-xs font-bold text-charcoal px-1">
                <span className="w-1/4 text-left">{content.xAxisLabels[0]}</span>
                <span className="w-1/4 text-center">{content.xAxisLabels[1]}</span>
                <span className="w-1/4 text-center text-high-vis-orange">{content.xAxisLabels[2]}</span>
                <span className="w-1/4 text-right">{content.xAxisLabels[3]}</span>
            </div>
            ...
            <div className="absolute top-[37.5%] left-[50%] md:left-[60%] -translate-x-1/2 w-[35%] md:w-[20%] h-[10%] bg-high-vis-orange flex items-center justify-center shadow-sm z-10">
               <span className="font-sans font-bold text-[10px] md:text-sm text-white whitespace-nowrap">{content.voidLabel}</span>
            </div>

            {/* Context Lost Annotation */}
            <div className="absolute top-[58%] left-[55%] md:left-[54%] transform">
               <div className="font-handwriting text-sm md:text-lg text-[#CC0000] rotate-[-5deg] whitespace-nowrap">{content.contextLostLabel}</div>
            </div>

            {/* Starting From Scratch */}
            <div className="absolute top-[85%] left-[75%] md:left-[85%] -translate-x-1/2 w-32 md:w-auto text-center">
               <span className="font-mono text-[9px] md:text-[10px] text-gray-500 bg-white/50 px-1">{content.startLabel}</span>
            </div>
            ...
            <div className="absolute top-[-2%] left-[2%] md:left-[8%] w-auto bg-white border border-charcoal p-2 md:p-3 shadow-lg hidden sm:block rotate-[-2deg]">
                <div className="font-sans text-[10px] md:text-xs font-bold text-charcoal whitespace-nowrap">{content.topCard}</div>
            </div>
            ...
            <div className="absolute bottom-[20%] right-[5%] w-32 md:w-48 bg-white border border-gray-300 p-2 md:p-3 shadow-sm hidden sm:block">
                <div className="font-mono text-[8px] md:text-[10px] text-gray-500 mb-1">{content.bottomCardLabel}</div>
                <div className="font-sans text-[10px] md:text-xs text-gray-400">{content.bottomCardQuote}</div>
            </div>
